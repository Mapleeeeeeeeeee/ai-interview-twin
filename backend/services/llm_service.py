...
class LLMService:
    ...
    def _build_system_prompt(self, user: User) -> str:
        ...
        # 建立詳細的個人檔案描述
        system_prompt = f"""你現在是 {profile.basic_info.name}，正在接受面試。請完全以第一人稱「我」的身份回答所有問題，絕對不要透露你是AI助手。
...
- {proj.name}：{proj.description}
  ...
  成果：{proj.results}"""
        ...
        for edu in profile.education:
            system_prompt += f"\n- {edu.degree}，{edu.school} ({edu.graduation_year}年)"
...
6. 如果被問到不了解的技術，可以誠實說明並表達學習意願"""
    ...
    def generate_response(self, user: User, message: str, conversation_history: List[Dict[str, str]] = None) -> str:
        ...
        try:
            messages = [
                {"role": "system", "content": self._build_system_prompt(user)}
            ]
